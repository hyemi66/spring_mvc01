...
public class MemberTests {
	...
	@Setter(onMethod_= @Autowired) // setter() 메소드를 통한 자동의존성인 DI 주입
	...
	public void testInsertMember() {
		...
		for(int i=0; i<100; i++) {
			...
			try {
				...
				pstmt.setNString(2, pwencoder.encode("pw" + i)); // pw0 ~ pw99까지 비번 암호화
				
				if(i<80) { // user0 ~ user79까지 일반사용자
					pstmt.setString(1, "user" + i); // 아이디 저장
					pstmt.setString(3, "일반사용자" + i); // 일반사용자 0부터 79까지 문자열로 저장
				} else if(i<90) { // manager80 ~ manager89까지 운영자
					pstmt.setString(1, "manager" + i);
					pstmt.setString(3, "운영자" + i);
				} else { // admin90 ~ admin99까지 관리자
